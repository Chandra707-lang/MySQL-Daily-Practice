...
-- in operator will check every value return by value by subquery
...
-- find the customers who never placed the orders
select * from customers where exists
( select 1 from orders where orders.customer_id! = customers.customer_id);
...
-- study all topics for tomorrow (ddl, int ,float,decimal,varchar,char,constraint, primary key, unique key, default and check)
