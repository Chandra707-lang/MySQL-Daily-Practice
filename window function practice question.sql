...
select customer_id,customer_name,order_id,order_date,row_number() over(partition by customer_id order by order_date) from orders;

select order_id,customer_id,amount, row_number() over(order by amount desc) from orders;

select category,order_id,amount, rank() over(partition by category order by amount desc) from orders;

select order_id ,category,amount,avg(amount) over(partition by category),amount-avg(amount) over(partition by category) from orders;

select * from 
(select customer_name,order_id,amount,max(amount) over(partition by customer_id  ) as cus_max 
from orders) as t where amount=cus_max;
-- or
select * from 
(select customer_name,order_id,amount,rank() over(partition by customer_id order by amount desc ) as cus_rank
from orders) as t where cus_rank=1;


select * from 
(select customer_name,order_id,amount,rank() over(partition by customer_id order by amount desc ) as cus_rank
from orders) as t where cus_rank in (1 ,2);

select * from 
(select customer_name,order_id,amount,rank() over(partition by customer_id order by amount desc ) as cus_rank
from orders) as t where cus_rank =2;

select customer_name,order_id,amount,
sum(amount) over(partition by customer_name) as total_spen,
round((amount*100)/sum(amount) over(partition by customer_name),2) from orders;