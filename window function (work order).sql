...
select sum(sales_amount) from employee_performance where location='jaipur';
show tables;
...
select *,sum(sales_amount) over(),sum(sales_amount) over (partition by location) from employee_performance;
select *,sum(sales_amount) over(),sum(sales_amount) over (partition by location order by sales_amount) from employee_performance;